...
const { defaultOptions, getModule, fetchFile } = require("./node");
...
module.exports = (_options = {}) => {
  let runResolve = null;
  let running = false;
  const { log: logging, logger, progress, ...options } = resolvePaths({
    ...defaultOptions,
    ..._options,
  });
  ...
  const writeText = (path, text) => FS("writeFile", [path, text]);

  const read = (path) => FS("readFile", [path]);

  const remove = (path) => FS("unlink", [path]);
